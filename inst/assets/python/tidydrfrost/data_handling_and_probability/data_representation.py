...
@task("explorer.php?noid=172", 0)
def pictograms(questions_answered):
...
@task("explorer.php?noid=172", 3)
def bank_statements(questions_answered):
...
@task("explorer.php?noid=172", 4)
def pie_charts(questions_answered):
  if s(".answer-content").elements("table").size() > 0:
    ...
    answers = map(lambda x: sigfig.round(round(x/total * 360, 8), decimals = 0), numbers)
